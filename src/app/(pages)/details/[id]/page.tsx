'use client';

import React, { useEffect, useState } from 'react';
import Image from 'next/image';
import { useParams } from 'next/navigation';
import { apiClient } from "@/libs/network";

import {
    FaFacebookF,
    FaInstagram,
    FaTwitter,
    FaWhatsapp,
    FaShippingFast,
    FaStore,
    FaHeart,
    FaBarcode,
    FaHashtag,
    FaShieldAlt,
} from "react-icons/fa";


const BE_URL = "https://api.bestbuyelectronics.lk";

interface Product {
    id: number;
    name: string;
    sku: string;
    model_number?: string;
    price: string;
    old_price?: string;
    quantity: number;
    warranty?: string;
    delivery_available: boolean;
    category: string;
    subcategory?: string;
    image_url?: string;
    description?: string;
    images?: string[];
}

const loadProductById = async (id: number | string): Promise<Product> => {
    const product = await apiClient.get<Product>(`products/${id}/`);
    return product;
};

const ItemView: React.FC = () => {
    const { id } = useParams();
    const [selectedImage, setSelectedImage] = useState<string>('');
    const [details, setDetails] = useState<Product>({
        id: 0,
        name: '',
        sku: '',
        model_number: '',
        price: '0.00',
        old_price: '0.00',
        quantity: 0,
        warranty: '0',
        delivery_available: false,
        category: '',
        subcategory: '',
        image_url: '',
        description: '',
        images: [],
    });

    useEffect(() => {
        if (!id) return;
        const productID = Number(id);
        if (isNaN(productID)) return;

        loadProductById(productID)
            .then((res) => setDetails(res))
            .catch((err) => console.error('Error loading product:', err));
    }, [id]);

    useEffect(() => {
        if (details.images && details.images.length > 0) {
            setSelectedImage(details.images[0]);
        }
    }, [details.images]);

    return (
        <div className="max-w-6xl mx-auto p-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                {/* Left Column - Images and Mobile Info */}
                <div>
                    {/* Main Image */}
                    <div className="relative w-full h-[250px] sm:h-[300px] rounded-xl overflow-hidden">
                        {selectedImage && (
                            <Image
                                src={`${BE_URL}${selectedImage}`}
                                alt="Main Product Image"
                                fill
                                className="object-contain p-4"
                                sizes="(max-width: 768px) 100vw, 50vw"
                                unoptimized
                            />
                        )}
                    </div>

                    {/* Thumbnails */}
                    <div className="flex gap-3 mt-3 overflow-x-auto">
                        {details.images?.map((img, idx) => (
                            <div
                                key={idx}
                                className={`relative w-20 h-20 flex-shrink-0  rounded-md cursor-pointer ${
                                    selectedImage === img ? '' : ''
                                }`}
                                onClick={() => setSelectedImage(img)}
                            >
                                <Image
                                    src={`${BE_URL}${img}`}
                                    alt={`Thumbnail ${idx + 1}`}
                                    fill
                                    className="object-cover rounded-md"
                                    sizes="80px"
                                    unoptimized
                                />
                            </div>
                        ))}
                    </div>

                    {/* Mobile Only - Name & Price */}
                    <div className="mt-5 md:hidden space-y-2">
                        <h1 className="text-lg font-semibold text-gray-900">{details.name}</h1>
                        <div className="flex items-baseline space-x-2">
                            <span className="text-xl font-bold text-green-600">
                                Rs. {Number(details.price).toLocaleString()}
                            </span>
                            {details.old_price && (
                                <span className="text-sm line-through text-gray-400">
                                    Rs. {Number(details.old_price).toLocaleString()}
                                </span>
                            )}
                        </div>
                    </div>

                    {/* Mobile Only - Additional Info */}
                    <div className="flex md:hidden flex-col space-y-6 mt-8">

                        <div className="space-y-5">

                            {/* Model Number */}
                            <div className="flex items-center space-x-3">
                                <FaBarcode className="text-blue-500" size={20} />
                                <div>
                                    <p className="text-xs text-gray-500 uppercase tracking-wide">Model Number</p>
                                    <p className="text-sm font-bold text-gray-800">{details.model_number || 'N/A'}</p>
                                </div>
                            </div>

                            {/* SKU */}
                            <div className="flex items-center space-x-3">
                                <FaHashtag className="text-purple-500" size={20} />
                                <div>
                                    <p className="text-xs text-gray-500 uppercase tracking-wide">SKU</p>
                                    <p className="text-sm font-bold text-gray-800">{details.sku || 'N/A'}</p>
                                </div>
                            </div>

                            {/* Warranty */}
                            <div className="flex items-center space-x-3">
                                <FaShieldAlt className="text-red-500" size={20} />
                                <div>
                                    <p className="text-xs text-gray-500 uppercase tracking-wide">Warranty</p>
                                    <p className="text-sm font-bold text-gray-800">
                                        {details.warranty ? `${details.warranty} Month(s)` : 'N/A'}
                                    </p>
                                </div>
                            </div>

                            {/* Delivery Info */}
                            <div className="flex items-center justify-end space-x-2 text-green-600 border-t border-gray-200">
                                <FaShippingFast size={16} />
                                <div className="text-right">
                                    <p className="text-xs text-gray-500">Standard Delivery</p>
                                    <p className="text-sm font-semibold text-green-600">3 - 5 Working Days Available</p>
                                </div>
                            </div>

                            <div className="flex items-center justify-end space-x-2 text-green-600">
                                <FaStore size={16} />
                                <div className="text-right">
                                    <p className="text-xs text-gray-500">Pickup In-Store</p>
                                    <p className="text-sm font-semibold text-green-600">Today Available</p>
                                </div>
                            </div>

                            {/* Wishlist Button */}
                            <button className="w-full flex items-center justify-center gap-2 py-2 rounded-md border border-gray-300 text-gray-700 text-sm hover:bg-gray-100 transition">
                                <FaHeart size={18} />
                                Add to Wishlist
                            </button>

                            {/* Social Links */}
                            <div className="flex justify-center space-x-6 pt-3 border-t border-gray-200">
                                <a href="#" aria-label="Facebook" className="text-gray-600 hover:text-blue-600 transition">
                                    <FaFacebookF size={18} />
                                </a>
                                <a href="#" aria-label="Instagram" className="text-gray-600 hover:text-pink-600 transition">
                                    <FaInstagram size={18} />
                                </a>
                                <a href="#" aria-label="Twitter" className="text-gray-600 hover:text-blue-400 transition">
                                    <FaTwitter size={18} />
                                </a>
                                <a href="#" aria-label="WhatsApp" className="text-gray-600 hover:text-green-600 transition">
                                    <FaWhatsapp size={18} />
                                </a>
                            </div>

                        </div>
                    </div>



                    {/* Description */}
                    <div className="mt-6">
                        <h2 className="text-lg font-semibold text-gray-800 mb-2">Product Description</h2>
                        <div
                            className="text-gray-600 text-sm leading-relaxed ql-editor"
                            dangerouslySetInnerHTML={{ __html: details.description || '' }}
                        />
                    </div>
                </div>

                {/* Right Column - Desktop only (no change) */}
                <div className="hidden md:flex flex-col max-w-xs ml-auto space-y-6">
                    <div className="text-center">
                        <h1 className="text-lg font-semibold text-gray-900 truncate">{details.name}</h1>
                        <div className="flex items-baseline justify-center space-x-3 mt-1">
      <span className="text-xl font-bold text-green-600">
        Rs. {Number(details.price).toLocaleString()}
      </span>
                            {details.old_price && (
                                <span className="text-sm line-through text-gray-400">
          Rs. {Number(details.old_price).toLocaleString()}
        </span>
                            )}
                        </div>
                    </div>

                    <div className="space-y-4">

                        {/* Model Number */}
                        <div className="flex items-center space-x-3">
                            <FaBarcode className="text-blue-500" size={20} />
                            <div>
                                <p className="text-xs text-gray-500 uppercase tracking-wide">Model Number</p>
                                <p className="text-sm font-bold text-gray-800">{details.model_number || 'N/A'}</p>
                            </div>
                        </div>

                        {/* SKU */}
                        <div className="flex items-center space-x-3">
                            <FaHashtag className="text-purple-500" size={20} />
                            <div>
                                <p className="text-xs text-gray-500 uppercase tracking-wide">SKU</p>
                                <p className="text-sm font-bold text-gray-800">{details.sku || 'N/A'}</p>
                            </div>
                        </div>

                        {/* Warranty */}
                        <div className="flex items-center space-x-3">
                            <FaShieldAlt className="text-red-500" size={20} />
                            <div>
                                <p className="text-xs text-gray-500 uppercase tracking-wide">Warranty</p>
                                <p className="text-sm font-bold text-gray-800">
                                    {details.warranty ? `${details.warranty} Month(s)` : 'N/A'}
                                </p>
                            </div>
                        </div>

                        {/* Delivery Info */}
                        <div className="flex items-center justify-end space-x-2 text-green-600 border-t border-gray-200">
                            <FaShippingFast size={16} />
                            <div className="text-right">
                                <p className="text-xs text-gray-500">Standard Delivery</p>
                                <p className="text-sm font-semibold text-green-600">3 - 5 Working Days Available</p>
                            </div>
                        </div>

                        <div className="flex items-center justify-end space-x-2 text-green-600">
                            <FaStore size={16} />
                            <div className="text-right">
                                <p className="text-xs text-gray-500">Pickup In-Store</p>
                                <p className="text-sm font-semibold text-green-600">Today Available</p>
                            </div>
                        </div>

                        <button className="w-full flex items-center justify-center gap-2 py-2 rounded-md border border-gray-300 text-gray-700 text-sm hover:bg-gray-100 transition">
                            <FaHeart size={18} />
                            Add to Wishlist
                        </button>

                        {/* Social Links */}
                        <div className="flex justify-center space-x-6 pt-3 border-t border-gray-200">
                            <a href="#" aria-label="Facebook" className="text-gray-600 hover:text-blue-600 transition">
                                <FaFacebookF size={18} />
                            </a>
                            <a href="#" aria-label="Instagram" className="text-gray-600 hover:text-pink-600 transition">
                                <FaInstagram size={18} />
                            </a>
                            <a href="#" aria-label="Twitter" className="text-gray-600 hover:text-blue-400 transition">
                                <FaTwitter size={18} />
                            </a>
                            <a href="#" aria-label="WhatsApp" className="text-gray-600 hover:text-green-600 transition">
                                <FaWhatsapp size={18} />
                            </a>
                        </div>
                    </div>
                </div>
            </div>


        </div>


    );
};

export default ItemView;
